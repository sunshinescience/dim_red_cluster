...
# Visualize a single image from the dataset. Input what image number you want to display in the index in digits.images[] below
sample_n = 32 # Sample you want to visualize
if 0: # To display an image from the dataset, type 1 instead of 0 in this if statement
...
if 1:
    ...
    plt.tight_layout()
    # plt.show()  
    plt.savefig('handwritten_digits_each_5.png', dpi=150) 
...
if 0:
    ...
    plt.tight_layout()
    plt.show()
    # plt.savefig('handwritten_digits_36random.png', dpi=150)
...
if 1:
    ...
    plt.tight_layout()
    plt.show()  
    # plt.savefig('handwritten_digits_10example.png', dpi=150) 

# #############################################################################
# PCA-reduced data

# From the original 64 dimensions, the first two principal components are generated below
...
# The number of dimensions are reduced via PCA and will subsequently be reduced via t-SNE
n_components = 20
pca_for_t_sne = PCA(n_components=n_components)
pca_result_for_t_sne = pca_for_t_sne.fit_transform(data) 
...
# In this case the seeding of the centers is deterministic, thus the kmeans algorithm is run only once with n_init=1
...
# Plot of PCA vs t-SNE reduced data and K-means clustering on both
color = k_t_sne.labels_
if 0: 
...
if 1: 
    ...
    # Plot of pca reduced data 
    ax1 = axarr[0]
    ax1.scatter(pca_result[:,0], pca_result[:,1], c='k', marker='.')
...
if 1:
    ...
    plt.scatter(iso_result[:, 0], iso_result[:, 1], c=target, cmap=plt.cm.get_cmap('jet', 10))
    plt.colorbar(ticks=range(10))
    plt.clim(-0.5, 9.5)
    plt.show() 
...
# Plot an elbow curve to select the optimal number of clusters (n_clusters) for k-means clustering
# Fit KMeans and calculate sum of squared errors (SSE) for each k, which is defined as the sum of the squared distance between each member of the cluster and its centroid.
sse = {}
for k in range(1, 20): 
